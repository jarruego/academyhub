...
| `docs/sms.md` | **touching `api/sms/`**, the SMS admin panel, or the SMS button on the group screen. Covers Mailrelay API integration (`X-AUTH-TOKEN`, E.164 phone normalization, no webhook → manual status refresh), `sms_settings`/`sms_templates`/`sms_log`, and the shared mail template-variable catalog. |
