...
export const PERMISSIONS_MATRIX_SECTIONS: PermissionMatrixSection[] = [
  ...
  },
  {
    ...
    rows: [
      ...
      { label: 'Plan de formación: listar/añadir (base o por centro)/quitar (del todo o solo del base), propio de cada consultoría anual', access: adminConsultor(), note: 'Borrado bloqueado si algún centro afectado ya evaluó la acción; al abrir una consultoría se clona del ejercicio anterior' },
      ...
      { label: 'Competencias y Puestos de trabajo (catálogos): listar', access: adminConsultor() },
      { label: 'Competencias y Puestos de trabajo: crear/editar/borrar', access: adminOnly(), note: 'Mismo criterio que categorías de curso; tocar una competencia pide además reconfirmar la contraseña (afecta a todas las plantillas)' },
      { label: 'Grupos de puestos de trabajo (catálogo): listar', access: adminConsultor() },
      { label: 'Grupos de puestos de trabajo: crear/editar/borrar', access: adminOnly(), note: 'Antes era texto libre en cada puesto (group_label), pasó a entidad propia; borrado bloqueado si algún puesto lo usa' },
      { label: '[Temporal] Autorrelleno de catálogo / automapeo de puestos', access: adminOnly(), note: 'Herramienta de un solo uso, se retira cuando el usuario avise' },
