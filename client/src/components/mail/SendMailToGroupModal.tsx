import { App, Modal, Form, Select, Radio, Button, Typography, Input, Divider, Collapse, Checkbox, Progress, Alert, Space, Tooltip, theme } from 'antd';
...
export default function SendMailToGroupModal({ open, users, tutors = [], courseName, groupStart, groupEnd, onOk, onCancel }: SendMailToGroupModalProps) {
  ...
  const handleSendTest = async () => {
    ...
    const replyTo = fromChoice === 'auth' ? authEmail : fromChoice === 'tutor' ? selectedTutor?.email : smtp?.from_email;
    const fromName = fromChoice === 'auth' ? authInfo?.user?.name : fromChoice === 'tutor' ? selectedTutor?.name : undefined;
