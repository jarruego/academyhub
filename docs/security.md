...
- `can_manage_candidates` — candidate-management access to one course edition: full edit on the **Planificación y selección** and **Candidatos** tabs (client-side, not a guarded endpoint — see `docs/course-catalog.md`), plus importing the official INAEM Preinscripciones file. For the import specifically see `docs/import-inaem.md`'s "Endpoints" for the exact split (ADMIN/MANAGER unrestricted; flag-only access forced to `restrictToFileNumber` + `createMissingCourses: false`).
