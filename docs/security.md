...
- `can_import_inaem` — import the official INAEM Preinscripciones file, scoped to one edition. See `docs/import-inaem.md`'s "Endpoints" for the exact split (ADMIN/MANAGER unrestricted; flag-only access forced to `restrictToFileNumber` + `createMissingCourses: false`).
