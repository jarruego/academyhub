...
| Plan de formación (`consulting_plan_items`): listar / añadir (base o a cada centro individualmente) / quitar (del todo, o solo del base manteniendo copias por centro) | ✅ | ❌ | ❌ | ❌ | ✅ | Mismo `ConsultingClientController`, `[ADMIN, CONSULTOR]` — solo admite acciones ya etiquetadas (`consulting_action_details`); borrado bloqueado si algún centro afectado ya evaluó la acción, salvo que se pida mantenerla en cada centro |
