...
Nombre, **nombre corto** (`short_name`, obligatorio, migración `0099`), código interno opcional, descripción, objetivos, **a quién va dirigido** (`target_audience`, añadido 2026-09-15 a petición de Consultoría — ver `docs/consultoria.md`), contenidos base, modalidad/horas habituales, referencia de especialidad SEPE y familia/área profesional. Los valores habituales son informativos: no reescriben ediciones existentes.

`catalog_courses.short_name` es distinto de `internal_code` (código, no nombre). Se pensó a nivel de catálogo porque cuenta cada carácter en SMS: variable de plantilla `{NOMBRE_CURSO_CORTO}` (ver `docs/sms.md`), resuelta desde el join `catalog_courses` de `CourseRepository.findById` (`catalog_course_short_name`) y propagada por el cliente hasta `SendSmsToGroupModal`. Al añadirse sobre una tabla ya poblada, la migración `0099` hizo backfill con el propio `name` — las entradas antiguas deben revisarse/acortarse manualmente desde la ficha del catálogo antes de usarse en un SMS real. Las altas automáticas (`ensurePendingByName`, Moodle/INAEM sin catálogo explícito) arrancan igual, con el mismo placeholder.

**`courses.short_name` (nombre corto de la edición) se eliminó** (migración `0100`, 2026-09-18): existía duplicado con el de catálogo y todos sus usos reales pasaron a `catalog_course_short_name` — búsqueda libre de ediciones (`courses.route.tsx`), duplicador de foros (`ForumDuplicator.tsx`) y export CSV de SMS (`use-export-users-sms-csv.tsx`); la columna de "Nombre Corto" en la pestaña de cursos de un usuario se quitó sin sustituto (no aportaba nada que no diera ya "Curso"). La sincronización con Moodle (`upsertMoodleCourse`, en `course.service.ts` y `moodle.service.ts`) y la importación INAEM ya no escriben ningún nombre corto de edición.
...
- Listado `/courses` (`courses.route.tsx`, «Ediciones»): orden por defecto por `Fecha Fin Grupo` descendente (sin fecha al final) y, a igualdad, `id_course` descendente. Columnas reducidas a lo esencial (Curso de catálogo, Nº Exp./Cliente/Financiación según pestaña, Fecha Fin Grupo, Estado) — sin ID, ID Moodle ni Nombre (siguen siendo consultables desde la ficha de la edición); la búsqueda libre casa por nombre y nombre corto del **catálogo** (`catalog_course_short_name`, ya venía en el mismo join), no por ningún nombre corto de la edición (columna eliminada, ver arriba). Mismo criterio en la búsqueda libre del servidor (`CourseRepository.findAll`, `filter.search`).
...
- `GET /counts` — nº de candidaturas agrupado por `id_course` (`[{id_course, count}]`), para todas las ediciones que tengan al menos una. Usado por el dashboard de Home (ver `docs/client.md` "Dashboard de Home") para decidir si mostrar el botón "Candidatos" de una edición sin traer el listado completo de cada una.
...
**Alerta de posible duplicado al dar de alta a alguien nuevo** (no al seleccionar del buscador): el buscador sigue siendo la vía principal — rápida para el caso común de que la persona ya se conozca — pero es una búsqueda por subcadena, no difusa, así que no detecta errores de tecleo ni apellidos en otro orden. Como red de seguridad, al pulsar "Aceptar" sin haber seleccionado a nadie, `PersonSearchOrCreateModal` ejecuta `findPotentialDuplicate` (`client/src/utils/duplicate-match.util.ts`) contra `availableUsers` (la misma lista que ya usa el buscador, sin nueva llamada al backend): coincidencia **exacta** de DNI/email/teléfono normalizados, o si no, similitud de **nombre** por Levenshtein (umbral `0.9`, longitud mínima `3` — mismo criterio que el import de SAGE, `findSimilarUsers`/`SIMILARITY_CONFIG` en `docs/import.md`, reimplementado en el cliente sin añadir esa dependencia). Si hay match, en vez de enviar se abre `DuplicateMatchModal` (`components/common/DuplicateMatchModal.tsx`, adaptación del grid clicable de `DecisionModal` del import SAGE) con tres acciones:
- **Usar los datos ya existentes** (por defecto): ignora lo escrito, `onSubmit({ id_user })` — igual que si se hubiera seleccionado del buscador.
- **Combinar campo a campo**: grid Nombre/Apellido1/Apellido2/DNI/Teléfono/Email, clicable por celda ("Nuevo" vs "Existente"), con highlighting de diferencias y una selección por defecto razonable (prioriza el lado no vacío). Al confirmar, aplica el patch elegido con `PUT /user/:id` (misma vía que la edición inline de la tabla de Candidatos) y luego `onSubmit({ id_user })`.
- **Son personas distintas — crear de todas formas**: sigue el camino normal (`new_user`), por si la similitud es una falsa alarma.

Al vivir en el componente compartido, cubre **Candidatos e Interesados a la vez** sin tocar `CourseCandidatesSection`/`CourseInterestsSection` ni los DTOs/servicios de `course-candidate`/`course-interest` — siempre resuelven a `{id_user}` o `{new_user}` como antes. El backend de `PUT /user/:id` (usado por "combinar") respeta el permiso puntual `can_manage_candidates` — ver `docs/security.md`.
...
**Importar Preinscritos INAEM** (antes "Importar Excel", reconvertido): visible para ADMIN/MANAGER o cualquiera con el permiso puntual `can_manage_candidates` (independiente del rol — ver `docs/security.md`). Ya no es un simple emparejado por DNI contra usuarios existentes: sube el fichero **oficial** de Preinscripciones INAEM al importador real (`docs/import-inaem.md`), con aviso previo + reautenticación por contraseña. Quien no sea ADMIN/MANAGER (solo tiene el flag) queda acotado al Nº de Expediente de esta edición — no puede tocar otros cursos ni crearlos.

`canEdit` que recibe `CourseCandidatesSection` se fija en `CourseDetailRoute` como `canEditCandidates = [ADMIN, MANAGER, TUTOR].includes(role) || can_manage_candidates` — variable propia de esta pestaña, distinta del `canEdit` general de la ficha del curso (`[ADMIN, MANAGER]`, usado en el resto de la Ficha: campos generales, "Guardar Curso", "Eliminar Curso"). La pestaña **Planificación y selección** usa su propia `canEditPlanning = canEdit || can_manage_candidates`: sigue reservada a ADMIN/MANAGER salvo que el actor tenga el permiso puntual, en cuyo caso también puede editarla aunque su rol no sea ADMIN/MANAGER/TUTOR — es la otra mitad del alcance de `can_manage_candidates` (gestión de candidaturas), junto con la edición completa de Candidatos y el import de Preinscritos INAEM. La Ficha incluye un botón "Ver" (icono, con tooltip "Ver contenidos del curso de catálogo") que abre en pestaña nueva la pestaña Contenidos del curso de catálogo (`catalog-course-detail.route.tsx`, ver "Datos del catálogo" arriba) — la edición ya no tiene contenidos propios. Es una excepción deliberada a la convención de clic simple = misma pestaña (`openDetail` directo, no `useLinkNavigation`) — ver `docs/client.md` "Table navigation (click convention)".
...
- Pestaña **Interesados** en la ficha del curso de catálogo (`course-interests-section.tsx`): misma filosofía tipo hoja de cálculo que Candidatos, incluido el **autoguardado** (sin botón "Guardar cambios", sin borrador local): Estado/Origen/Modalidad preferida guardan al cambiar de valor (`Select` → `onChange`) y Disponibilidad/Notas guardan al perder el foco vía el mismo `AutoSaveText` compartido que usa Candidatos. `useUpdateCourseInterestsMutation(catalogCourseId)` es **optimista** igual que en Candidatos: aplica el patch a la caché (`onMutate`, clave `["course-interests","catalog",id]`) antes de que responda el servidor, revierte en `onError` y siempre invalida en `onSettled`. "Añadir interesado" usa el mismo `PersonSearchOrCreateModal` compartido que Candidatos (`components/common/`) — buscar existente o dar de alta a alguien nuevo en el mismo formulario. `POST /course-interests` acepta `{ id_user, id_catalog_course, ... }` o `{ id_catalog_course, new_user: {...}, ... }`, igual patrón que `POST /course-candidates`.
  - `CourseInterestsSection` distingue `canEdit` (ADMIN/MANAGER: editar campos de una fila existente, exportar Excel) de `canAdd` (ADMIN/MANAGER/TUTOR: botón "Añadir interesado" + modal, **y** el botón de eliminar en Acciones — alta/baja del listado), fijado en `CatalogCourseDetailRoute` (`canAddInterests`). Sin `canAdd` explícito el prop hace de alias de `canEdit` (mismo comportamiento en cualquier otro consumidor del componente). No hay importación de Excel aquí (a diferencia de Candidatos, ver arriba) — se quitó por no usarse.
