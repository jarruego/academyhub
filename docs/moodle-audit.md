...
## Reparación
- **Vínculos incorrectos → fusión**: la UI reutiliza la herramienta de Fusión de duplicados (`docs/user-merge.md`): "Fusionar →" abre el `MergeModal` (exportado ahora desde `MergeDuplicates.tsx`) con ganador = usuario por DNI y perdedor = vinculado. `useMergeMutation` invalida también `["moodle-audit-report"]`.
