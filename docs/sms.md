...
- **`POST /sms/preview-batch`** (mismos guards que `send`): la comprobación previa de `preview-length` pero para **todos** los destinatarios de golpe, no solo el primero — `SmsService.previewBatch` recibe `userIds: number[]` (+ `templateId`/`message` y los datos de curso, igual que `preview-length`) y devuelve, por cada uno, `{userId, missingVariables, length, parts, exceedsLimit}`. Una sola consulta de cuentas Moodle para todos los `userIds` (`MoodleUserRepository.findByUserIds`, `inArray`), no una por destinatario. `SendSmsToGroupModal` la llama al pulsar "Enviar" (antes de mandar nada): si algún destinatario tiene un problema, muestra su nombre + motivo en un diálogo de confirmación y ofrece "Enviar solo al resto" (los omitidos se cuentan aparte en el resultado, "Omitidos (variables sin valor o SMS muy largo)") en vez de descubrirlo por el recuento de "Fallidos" al terminar. Añadido 2026-09-18 a petición del usuario. `buildTemplateVariables` se partió en dos (`buildVariablesFromMoodleUsers`, pura, + la envolvente que hace la consulta de un único alumno) para poder reutilizar la parte pura aquí sin repetir consultas.
