...
- **Informes** (`components/course-requests/course-request-report-tab.tsx`): filtros empresa (**multi-selección** — `Select mode="multiple"`)/centro/curso (combinables), tabla agrupada Empresa → Curso → Centro (mismas filas de `/report`, con `rowSpan` calculado en cliente para fusionar celdas repetidas de Empresa/Curso — sin depender de ninguna librería de árbol/pivote), tarjetas de totales (Empresas/Cursos/Centros/Alumnos/Peticiones), y botón "Exportar a PDF" (`use-course-request-report-pdf.mutation.ts`, descarga vía blob).
...
- Muestra las peticiones `ABIERTA` para el `id_course` del grupo (`GET /api/course-requests?id_course=X&status=ABIERTA`).
- El usuario selecciona una o varias peticiones (checkbox); los alumnos de todas se combinan y se deduplication por DNI normalizado.
- Se cruzan con `GET /user/lookup` (por DNI) para detectar qué alumnos ya existen en la BD.
- Tabla de alumnos con columna "En BD" y datos actuales: misma lógica que `ImportUsersToGroupModal` (los nuevos se crean via `bulkCreateAndAddToGroup`; los existentes se añaden via `bulkAddUsersToGroup`).
- Tras importar con éxito: las peticiones seleccionadas se cierran automáticamente (`PUT /:id/close`).
