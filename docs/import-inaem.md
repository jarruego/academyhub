...
- `courses.file_number` — nº de expediente, **UNIQUE** (NULLs don't collide; matching key + manual tagging). `courses.origin` enum `course_origin` (`CLIENTE`/`INAEM`/`PRIVADO`/`OTRO`). `courses.is_provisional` (autocreated stub, completed later).
...
- **Course matching**: `file_number` exact → else create. New/provisional courses get `origin=INAEM`. Existing matched course: fill-gaps for dates/hours/origin (never overwrites a manual origin); a provisional course gets its real name + `is_provisional=false` when Acciones arrives. Course dates come from `Inicio`/`Fin` (not `Mes-Año`); when dates are filled on a course they propagate to its groups that have empty dates.
