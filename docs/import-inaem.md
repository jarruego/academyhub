...
- `GET /last-import` → `{ completedAt }`, la fecha del último `import_jobs` de tipo INAEM con `status=COMPLETED` (`JobService.getLastCompletedJob`, reutilizable por tipo). **Sin `RoleGuard` propio** (solo el `AuthGuard` global) — deliberado: es un GET de solo lectura sin datos sensibles, y lo consume el dashboard Home ("Última importación INAEM" junto al cuadro de financiación pública, `docs/client.md`), visible a cualquier rol.
