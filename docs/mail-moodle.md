...
`pushGroupToMoodle` (client button "Subir a Moodle" on the group ficha) creates or updates the Moodle group. `RoleGuard([ADMIN, MANAGER])` — same as the member-level Moodle actions (`add-members` and `GroupUsersManager`'s "Moodle" dropdown); the client mirrors it with a separate `AuthzHide roles={[ADMIN, MANAGER]}` around just this button (deleting the group in Moodle, `groups/:groupId/delete`, stays `ADMIN`-only — it's destructive and has its own `AuthzHide roles={[ADMIN]}`). Two branches:
